...
{
    internal class ListXeKhach : List<XeKhach>
    {
        public void AddXe(XeKhach _xe)
        {
            if (XeKhach.IsValid(this, _xe))
            {
                Add(_xe);
            }
            else
            {
                Console.WriteLine(", Add Xe Khach Failed");
            }
        }
        public void RemoveByID(string _id)
        {
            foreach (XeKhach _xe in this)
            {
